...
function prepareShopifyData(data) {
  ...
  // out of stock variants showing price fix - Vlad fix 
  let prices = data.variants
  ...
  onSale = data.compare_at_price_min > minPrice; // Check a product is on sale
  priceVaries = minPrice != data.price_max; // Check a product has many prices
  // Convert images to array
  images = data.images_info;
  // Get First Variant (selected_or_first_available_variant)

  //const inStockVariants = data.variants.filter(variant => variant.inventoryQuantity > 0);
  //data['variants']
  ...
  firstVariant = data['variants'][0];
    ...
    for (var i = 0; i < data['variants'].length; i++) {
      if(data.variants.find(v => v.inventory_quantity > 0)){
...
ProductGridItem.prototype.compileTemplate = function(data) {
  ...
  // added by heartcoding
  
    if (data.available && data.compare_at_price_min > minPrice) {
        var discount = (data.compare_at_price_min - minPrice) * 100 / data.compare_at_price_min;
        if (discount != undefined){
          var tag = '<div class="hc-sale-tag"><span>'+discount.toFixed(0)+'%</span></div>';
          itemHtml = itemHtml.replace(/{{hcSaleTag}}/g, tag);
        }else{
          itemHtml = itemHtml.replace(/{{hcSaleTag}}/g, "");
        }
    }else{
      itemHtml = itemHtml.replace(/{{hcSaleTag}}/g, "");
    }
...
function buildPrice(data) {
  ...
  //data = data.variants.filter(v => v.available === true);
...
console.log(minPrice);
  if (boostPFSThemeConfig.custom.hasOwnProperty('show_price') &&
    ...
    if (onSale) {
      ...
      html += '<span class="boost-pfs-filter-product-item-sale-price">' + Utils.formatMoney(minPrice) + '</span>';
      html += '<s>' + Utils.formatMoney(data.compare_at_price_min) + '</s> ';
    } else {
      if (priceVaries && minPrice !== Infinity ) {
        html += '<span class="boost-pfs-filter-product-item-price-from-text">' + (boostPFSThemeConfig.label_basic.from) + ' ' + '</span>';
      }
      if (minPrice !== Infinity ){
      html += '<span class="boost-pfs-filter-product-item-regular-price">' + Utils.formatMoney(minPrice) + '</span>';
    }else{
      html += '<span class="boost-pfs-filter-product-item-regular-price"></span>';
    }
    }
    html += '</p>';
  }
...
jQ(document).ready(function() {
    ...
    jQ('.boost-pfs-filter-custom-drawer-close').on('click', function(){
        jQ('body').removeClass('boost-pfs-filter-custom-drawer-open');
    });
    jQ('.boost-pfs-filter-custom-drawer-overlay').on('click', function(){
        jQ('body').removeClass('boost-pfs-filter-custom-drawer-open');
    });
